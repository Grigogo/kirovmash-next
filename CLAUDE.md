...
- При создании или редактировании компонентов **никакого хардкода UI-текста**. Любая видимая строка — через `useTranslations` / `getTranslations` и ключ. Если нужного ключа в `messages/*.json` нет — добавить в оба файла одновременно.
- При работе с БД для отображения сущностей всегда передавать текущую локаль и подгружать перевод.

## Интернационализация (i18n)

- **Стек:** `next-intl` (для App Router).
- **Языки:** `ru` (русский) и `en` (английский), оба активны.
- **URL-структура:** все языки с префиксом — `/ru/...`, `/en/...`. Корневой путь `/` редиректит на язык браузера через `Accept-Language` (fallback на `ru`).
- **Файлы переводов:** `messages/ru.json`, `messages/en.json`. Ключи иерархические (например, `common.cta.contactUs`, `nav.catalog`, `home.hero.title`).
- **Структура `app/`:** все маршруты публичной части и админки находятся внутри сегмента `app/[locale]/`. API-роуты остаются в `app/api/` без локализации.
- **Конфиг:** `i18n/routing.ts` (список локалей, дефолтная), `i18n/request.ts` (загрузка сообщений), `middleware.ts` в корне (детект языка и редирект).
- **Использование в Server Components:** `getTranslations` из `next-intl/server`. В Client Components — хук `useTranslations`. Не использовать `next-intl/client` напрямую без `'use client'`.
- **Линки:** только через `Link` из `i18n/routing.ts` (обёртка над `next/link`, автоматически добавляет префикс локали).
- **Хардкод текста запрещён.** Любой видимый пользователю текст — через ключ переводов. Исключение — заведомо неязыковые строки (имена брендов, slug'и, технические идентификаторы).

## Многоязычный контент в БД

- Все пользовательские сущности с текстовыми полями (Category, Product, Service) хранят переводы в **отдельных таблицах** `CategoryTranslation`, `ProductTranslation`, `ServiceTranslation`.
- В основной модели остаются язык-независимые поля: `id`, `slug`, `images`, `price`, `published`, связи и т.д.
- В таблицах переводов — `locale` (`ru` | `en`), `name`, `description` и другие переводимые поля. Уникальность по паре `(entityId, locale)`.
- При выборке всегда подгружаем перевод для текущей локали через `include: { translations: { where: { locale } } }`.
- Если перевода для локали нет — fallback на русский.
- Для типобезопасности завести вспомогательные функции в `lib/i18n.ts`: `pickTranslation(entity, locale)`, чтобы не дублировать логику fallback'а.
- Slug **не** переводится — единый для всех языков (это упрощает SEO и логику роутинга, и для конвейерного оборудования это разумно: артикулы и категории интернациональны).

## Дизайн (палитра и стиль)

- **Палитра:** базовая нейтральная (белый/серый/чёрный) + оранжевый акцент для CTA, выделений, активных состояний.
- **Стиль:** строгий, индустриальный, без декоративных излишеств. Чёткая типографика, много воздуха, понятная иерархия.
- **shadcn theme:** стиль `new-york`, base color `neutral`, CSS variables on, accent `--primary` — индустриальный оранжевый (что-то в районе `oklch(0.65 0.18 45)` или близкое — точный оттенок выбираем при настройке).
- **Иконки:** `lucide-react`.
- **Шрифт:** системный Geist (по умолчанию из create-next-app).
